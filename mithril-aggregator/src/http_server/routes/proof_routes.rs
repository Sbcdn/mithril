...
struct CardanoTransactionProofQueryParams {
    ...
    /// snapshot (the historical behaviour). When set, the proof is served against the most
    /// recent snapshot certified at or below this block number, so a caller can prove a
    /// transaction against a chosen historical Mithril certificate instead of the moving tip.
    #[serde(default)]
...
mod handlers {
    ...
    pub async fn proof_cardano_transaction(
        ...
        // Serve against the snapshot certified at or below `up_to_block_number` when set,
        // otherwise the latest certified snapshot. A bound with no certified snapshot at or
        // below it returns 404 (no anchor); it never falls back to the latest tip.
    ...
    pub async fn proof_v2_cardano_transaction(
        ...
        // Serve against the v2 snapshot certified at or below `up_to_block_number` when set,
        // otherwise the latest. A bound below the first CardanoBlocksTransactions beacon (v2
        // only began signing recently) has no anchor and returns 404; never the latest tip.
        let signed_entity = match transaction_parameters.up_to_block_number {
            Some(block_number) => unwrap_to_internal_server_error!(
                signed_entity_service
                    .get_cardano_blocks_transactions_snapshot_at_or_below_block_number(BlockNumber(
                        block_number
                    ))
                    .await,

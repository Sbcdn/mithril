...
- Added support for the default incremental backend (v2) for Cardano database restoration in the client library, CLI, and WASM
- Added enhanced verification for Cardano database integrity, providing a list of tampered or missing files in case of failure, in the client library and CLI
- Added support for artifact retrieval by epoch for the Cardano database and stake distribution in the client library, CLI, and WASM
- Deprecated the `with_aggregator_client` and `new` functions in the `ClientBuilder` struct of the client library
- General bug fixes and performance improvements.

The distribution has been deployed to the **Mithril aggregator** on the `release-mainnet` and `release-preprod` networks.
